...
namespace LangYa:: inline AutoAim
{
	...
	struct ExtendedKalmanOld
	{
		...
		double ChiSquaredTest() //卡方检验
		{
			auto residual_xy = Eigen::Vector2d(residual[0], residual[1]);
			Eigen::Matrix2d measurement_cov_matrix_xy;
			measurement_cov_matrix_xy << measurement_cov_maxtrix(0, 0), measurement_cov_maxtrix(0, 1),
				measurement_cov_maxtrix(1, 0), measurement_cov_maxtrix(1, 1);

			return residual_xy.transpose() * measurement_cov_matrix_xy * residual_xy;
		}
	};
}
